...
class DataManager:
    ...
    def save_all_data(self):
        """
        Speichert alle gültigen Daten aus dem Session State in SWITCHdrive.
        """
        keys = [key for key in self.data_reg if key in st.session_state]
        for key in keys:
            try:
                self.save_data(key)
            except Exception as e:
                st.warning(f"⚠️ Fehler beim Speichern von '{key}': {e}")
